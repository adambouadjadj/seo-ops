---
name: weekly-report
description: Génère le reporting SEO hebdomadaire AB Croisière. Utiliser quand l'utilisateur veut créer le rapport hebdo, analyser les positions Monitorank, traiter le fichier GSC performance, générer le mail de reporting Outlook.
---

# Workflow Reporting SEO Hebdomadaire (AB Croisière)
...
Une fois le mail généré, produire deux fichiers dans un sous-dossier par semaine `output/reporting/DD-MM-YY/` :
- `output/reporting/DD-MM-YY/reporting_AB_DD-MM-YY.md` — version Markdown (source, archivage)
- `output/reporting/DD-MM-YY/reporting_AB_DD-MM-YY_email.html` — version HTML Outlook-ready (à ouvrir dans un navigateur, puis Ctrl+A / Ctrl+C / coller dans Outlook)
...
- Sections "Actions réalisées" et "Actions prévues" : toujours utiliser `<ul><li>` avec `style="margin:0;padding:0 0 0 20px;"` sur le `<ul>` — jamais de `<table>` ni de `<p>` pour lister des items
